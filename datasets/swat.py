...
class SWaTDataset(torch.utils.data.Dataset):
    def __init__(self, root="data", contents=None, raw = False):
        ...
        train_fn, test_fn = "swat/train_processed.csv", "swat/test_processed.csv"
        test_gt = 'swat/test_gt_exp.csv'
        raw_fn = 'swat/raw.csv'
        ...
        if contents == "all":
            ...
            self.data = pd.read_csv(test_path, index_col=0)
        ...
        if raw:
            self.data = pd.read_csv(raw_path, index_col=0)   
            test_explanation = pd.read_csv(test_exp_path)
    ...
    def __getitem__(self, index):
        ...
        x = torch.tensor(self.data.iloc[index, :-2].values)
...
def get_swat_sliding_dataloaders(window_size,
                                 ...
                                 train_frac = 0.7,
                                 seed = None):
  good_dataset = SWaTSlidingDataset(window_size=window_size, stride=stride, contents="train")
  anom_dataset = SWaTSlidingDataset(window_size=window_size, stride=stride, contents="valid")
