...
def run_squad(seeds=[0,1], num_todo=250, model_file="squad_epoch2.pt"):
...
def run_hai(seeds=[0,1], num_todo=250, model_file="lr_hai_epoch5.pt"):
  state_dict = torch.load(os.path.join(MODELS_DIR, model_file))
  model = LogisticRegression(in_shape=(86,), return_mode="two_class")
  model.load_state_dict(state_dict)
  dataset = HAIDataset(contents="valid")
  for i, seed in enumerate(seeds):
...
def run_wadi_sliding(seeds=[0,1], num_todo=250, model_file="wadi-sliding_epoch2.pt"):
  state_dict = torch.load(os.path.join(MODELS_DIR, model_file))
...
def run_swat_sliding(seeds=[0,1], num_todo=250, model_file="swat-sliding_epoch2.pt"):
  state_dict = torch.load(os.path.join(MODELS_DIR, model_file))
